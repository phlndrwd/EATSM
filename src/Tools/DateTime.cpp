...
std::string DateTime::ProduceCumulativeTimeString( ) {
    ...
    Types::StringStream splitTimeStream;
...
std::string DateTime::ProduceTotalTimeString( ) {

    EndTiming( );

    Types::StringStream totalTimeStream;
    totalTimeStream << "Total run time = " << Convertor::Get( )->NumberToString( mTotalTimeInSeconds ) << "s, ";

    return totalTimeStream.str( );
}

std::string DateTime::GetFormattedDateAndTime( std::string format, const bool isEnd ) const {
...
void DateTime::SetTimeNow( Types::TimeStruct* time ) const {
